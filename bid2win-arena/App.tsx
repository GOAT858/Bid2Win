...
const App: React.FC = () => {
  ...
  const [roomPlayers, setRoomPlayers] = useState<{name: string}[]>([]);
  ...
  const startGame = (players: {name: string}[]) => {
    ...
    // Simulate opponent rating for the session
    setOpponentRating((user?.rating || 100) + Math.floor(Math.random() * 200));
    setActivePage('GAME');
  };
